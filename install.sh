...
AUTO_UPDATE_LATEST_VERSION_ENDPOINT="https://relay.$BASE_DOMAIN/public/latest-client-version"
...
export AUTO_UPDATE_LATEST_VERSION_ENDPOINT=""
...
# Patch installed wrapper to ensure auto-updates are disabled
status "Patching wrapper to lock version 0.3.51..."
if [ -f "$BINDIR/inference" ]; then
    $SUDO sed -i 's|AUTO_UPDATE_LATEST_VERSION_ENDPOINT=.*|AUTO_UPDATE_LATEST_VERSION_ENDPOINT=""|g' "$BINDIR/inference"
    status "Wrapper patched - auto-updates disabled for version lock"
fi
